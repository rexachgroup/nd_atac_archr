...
out_dir <- file.path(base_dir, "07_archr_harmony_subclustering_dx")
...
main <- function() {
    ...
    cluster_args_tb <- tribble(
        ~region,    ~clusters,                  ~dx,            ~proj_name,
        "PreCG",    c("C6", "C7", "C8", "C9"),  c("AD"),        "precg-c6789-ad",
        "PreCG",    c("C6", "C7", "C8", "C9"),  c("bvFTD"),     "precg-c6789-ftd"
    )
...
subcluster_worker <- function(proj_dir, f_region, f_cluster, f_dx, out_path) {
    ...
    filtered_cells <- cell_data %>%
        filter(region %in% f_region & Clusters %in% f_cluster & Clinical.Dx %in% f_dx)
