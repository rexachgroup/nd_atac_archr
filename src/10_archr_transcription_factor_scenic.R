# TF / ChromVAR motif derivation using external scenic motif set.
...
out_dir <- file.path(base_dir, "10_tf_scenic")
...
main <- function() {
    ...
    print(class(pwm))
    names(pwm) <- name(pwm)
    pwm_indices <- unlist(map(unique(motif_tb$TF), function(tf) {
        grep(tf, name(pwm), value = TRUE, fixed = TRUE)
    }))
    pwm_indices <- pwm_indices[!duplicated(pwm_indices)]
    motif_pwmlist <- pwm[pwm_indices]
    write.csv(name(motif_pwmlist), file.path(out_dir, "filtered_motifs.csv"))

    clearRegistry()
    reg$packages <- liblist
    batchExport(mget(ls()))
    ids <- batchMap(tf_worker, args = list(proj_dir = args_tb$proj_dir, out_dir = args_tb$out_dir), more.args = list(motif_pwmlist = motif_pwmlist))
