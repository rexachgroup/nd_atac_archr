# TF / ChromVAR motif derivation using HOMER motif db.
...
out_dir <- file.path(base_dir, "10_tf_homer")
...
main <- function() {
    ...
    batchExport(mget(ls()))
    ids <- batchMap(tf_worker, args = list(proj_dir = args_tb$proj_dir, out_dir = args_tb$out_dir))
...
tf_worker <- function(proj_dir, out_dir, motif_pwmlist) {
    ...
    project <- addMotifAnnotations(project, motifSet = "homer", name = "Motif", force = TRUE)
